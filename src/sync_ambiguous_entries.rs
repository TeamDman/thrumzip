...
pub async fn sync_ambiguous_entries(
    destination: &Path,
    entries: HashMap<PathInsideZip, Vec<ZipEntry>>,
) -> Result<(), eyre::Error> {
    let mut progress = Progress::new(
        entries.values().flatten().count(),
    ...
    info!(
        "Beginning sync of {} ambiguous entries to {}",
        entries.len(),
        destination.display()
    );
    ...
    for (name, entries) in entries {
        let dest = get_splat_path(&name, &destination, todo!())?;
        join_set.spawn(async move {
            ...
            Ok(Response {
                skipped,
                written,
                size: todo!(), //Information::new::<byte>(entry.entry.uncompressed_size as f64),
                dest,
            })
        });
    }
